...
class PizzaTests(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        def load_relation(name):
            with open('fakerbase/tests/test_data/{}.json'.format(name)) as f:
                data = json.load(f)
            return Relation(data['attrs'], data['tuples'])
        cls.person = load_relation('person')
        cls.frequents = load_relation('frequents')
        cls.eats = load_relation('eats')
        cls.serves = load_relation('serves')
